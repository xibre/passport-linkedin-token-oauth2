...
LinkedinTokenStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.error) {
    console.log('LinkedinTokenStrategy: req.query.error is present', req.query.error);
    return this.fail();
  }
  ...
  if (!accessToken) {
    console.log('LinkedinTokenStrategy: can\'t find accessToken in request', accessToken);
    return this.fail();
  }

  self._loadUserProfile(accessToken, function(err, profile) {
    if (err) {
      console.log('LinkedinTokenStrategy: failed to load user profile by access token', accessToken, err);
      return self.fail(err);
    };

    function verified(err, user, info) {
      if (err) {
        console.log('LinkedinTokenStrategy: verification failed', accessToken, err);
        return self.error(err);
      }
      if (!user) {
        console.log('LinkedinTokenStrategy: user not found', accessToken, info);
...
LinkedinTokenStrategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this.profileUrl, accessToken, function (err, body, res) {
    if (err) {
      console.log('LinkedinTokenStrategy: failed to fetch user profile', this.profileUrl, accessToken, err);
    ...
    try {
      ...
      console.log('LinkedinTokenStrategy: failed to parse profile', this.profileUrl, accessToken, body, e);
      done(e);
    }
  });
}
